...
The `TaskController` process is the experiment orchestrator. It creates a `transitions.Machine` from the task's declared STATES and TRANSITIONS, wires `on_enter_<state>`/`on_exit_<state>` callbacks, and runs a main loop that polls ZeroMQ for haptic state and dispatches state machine triggers. Every state transition publishes an event on the bus and sends an event code through the SyncProcess. The production path selects between `HapticClient` (real delta.3, and mock C++ server for laptop development with `dhd.mouse_input=True`) and `MockHapticInterface` (CI/simulation) via the `backend` field of `HapticConfig`, using the `make_haptic_interface()` factory in `hapticore.haptic`. Laptop development uses `backend=dhd` with `dhd.mouse_input=True`, which starts a `MouseBridge` thread forwarding mouse positions to the mock C++ server. Both `HapticClient` and all mock interfaces satisfy the `HapticInterface` Protocol. The display path is selected analogously: `DisplayConfig.backend` chooses between `psychopy` (real `DisplayProcess` + `DisplayClient`) and `mock` (in-process `MockDisplay`), via the `make_display_interface()` factory in `hapticore.display`. The audio path is **in-process**: `AudioConfig.backend` chooses between `sounddevice` (real `AudioPlayer`, not yet implemented) and `mock` (in-process `MockAudio`), via `make_audio_interface()` in `hapticore.audio`. Audio has no subprocess or ZMQ topic — `play_cue()` is a direct method call in the `TaskController` process. With the current `mock` backend, calls are recorded for testing. The follow-up `sounddevice` backend will return immediately while PortAudio's callback thread handles output asynchronously.

**Hardware interface summary:**

| Interface | Protocol | Real backend | Mock backend | Transport |
|-----------|----------|-------------|-------------|-----------|
| `HapticInterface` | `hapticore.core.interfaces` | `HapticClient` (ZMQ to C++ server) | `MockHapticInterface` | ZMQ PUB/ROUTER |
| `DisplayInterface` | `hapticore.core.interfaces` | `DisplayClient` (ZMQ to `DisplayProcess`) | `MockDisplay` | ZMQ PUB |
| `SyncInterface` | `hapticore.core.interfaces` | `TeensySync` (ZMQ to `SyncProcess`) | `MockSync` | ZMQ PUB |
| `AudioInterface` | `hapticore.core.interfaces` | `AudioPlayer` (PortAudio, follow-up) | `MockAudio` | In-process (no ZMQ) |
