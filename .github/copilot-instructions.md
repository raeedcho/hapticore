...
- Libraries are NOT in `$FD_SDK_DIR/lib/`. They are in `$FD_SDK_DIR/lib/release/lin-<arch>-gcc/` on Linux (where `<arch>` is the machine architecture, e.g., `x86_64`), or `$FD_SDK_DIR/lib/release/mac-<arch>-clang/` on macOS (e.g., `mac-arm64-clang` on Apple Silicon). CMakeLists.txt uses `CMAKE_SYSTEM_PROCESSOR` and `APPLE` to resolve this automatically, matching the SDK's own Makefile convention (see `Makefile.common` in the SDK root).
- `libdhd.a` statically depends on `libusb-1.0` **on Linux only**. Since static libraries don't carry transitive dependencies, `usb-1.0` must be linked explicitly in `target_link_libraries` alongside `dhd` and `drd`. On macOS, the SDK uses Apple's native IOKit USB stack instead — link `IOKit` and `CoreFoundation` frameworks, not libusb.
...
- **Not every SDK-supported device implements every SDK call.** The Novint Falcon rejects `drdAutoInit()` and does not implement `dhdSetGravityCompensation()`, unlike the delta.3. Optional capabilities like these are declared per device in the rig config (`DhdConfig.auto_calibrate`, `DhdConfig.gravity_compensation`, passed to the server as `--no-calibrate` / `--no-gravity-comp`), never branched on device type in C++ — the same binary serves both devices.
