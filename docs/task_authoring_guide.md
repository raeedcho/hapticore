...
The `audio.backend` field is set in the rig config (`rig2.yaml` and `dev-mouse.yaml` use `sounddevice`; `ci.yaml` uses `mock`). Experiment YAMLs only need to specify `audio.cues`.
...
**Key behaviors:**
- `play_cue()` is fire-and-forget. It submits the buffer to PortAudio and returns immediately; audio output happens asynchronously on the callback thread. With the mock backend (CI), calls are recorded in `_play_log` for testing.
