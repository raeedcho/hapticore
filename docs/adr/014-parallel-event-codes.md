...
- `architecture.md:114` `SyncProcess` serial protocol: `E<code>` is now fully specified as an 8-bit value and can be documented as such. `architecture.md:200` updated to say "8-bit parallel event codes on the Scout D-sub port" instead of the currently vague "outputs event codes to both systems."
