...
│   ├── backends/       # haptic interface backends (HapticClient, mocks, mouse-driven)
...
- **Never send a single heartbeat before a blocking `input()` call.** The server reverts to NullField after 500 ms without a heartbeat. Use the `heartbeat_keeper` context manager (in `tests/hardware/heartbeat_keeper.py`) to keep forces alive.
